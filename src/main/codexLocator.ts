...
export async function locateCodexBinary(): Promise<string> {
  ...
  const explicit = process.env.LUDORA_CODEX_BIN ?? process.env.NOOBI_CODEX_BIN;
  ...
  throw new Error(
    `Unable to locate a runnable Codex binary. Set LUDORA_CODEX_BIN. Checked ${failures.length} candidate(s).`,
  );
}
