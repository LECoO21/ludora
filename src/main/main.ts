...
const smokeCapture = process.env.LUDORA_SMOKE_CAPTURE?.trim()
  || process.env.NOOBI_SMOKE_CAPTURE?.trim()
  || null;
const debugEnabled = process.env.LUDORA_DEBUG === '1' || process.env.NOOBI_DEBUG === '1';
if (smokeCapture) app.setPath('userData', resolve('.ludora-smoke/user-data'));
...
if (!hasSingleInstanceLock) {
  ...
  void app.whenReady().then(launch).catch((error) => {
    if (smokeCapture) process.stderr.write(`Ludora UI smoke failed: ${asError(error).message}\n`);
    else dialog.showErrorBox('Ludora 无法启动', asError(error).message);
    app.exit(1);
  });
...
async function createWindow(): Promise<void> {
  ...
  const rendererUrl = process.env.LUDORA_RENDERER_URL ?? process.env.NOOBI_RENDERER_URL;
...
function bindRuntimeEvents(): void {
  ...
  runtime.on('diagnostic', (message: string) => {
    if (debugEnabled) process.stderr.write(`[codex] ${message}\n`);
  });
  ...
  approvalBroker.on('diagnostic', (message: string) => {
    if (debugEnabled) process.stderr.write(`[approval] ${message}\n`);
  });
...
function emitAgentEvent(event: AgentEvent): void {
  void eventLog.append(event).catch((error) => {
    if (debugEnabled) process.stderr.write(`[event-log] ${asError(error).message}\n`);
  });
...
function handle(
  ...
  ipcMain.handle(channel, (event, ...args) => {
    ...
    if (shuttingDown) throw new Error('Ludora 正在退出');
...
function assertTrustedRenderer(event: IpcMainInvokeEvent): void {
  ...
  const expected = process.env.LUDORA_RENDERER_URL ?? process.env.NOOBI_RENDERER_URL;
...
async function captureSmoke(window: BrowserWindow, target: string): Promise<void> {
  ...
  if (!healthy) throw new Error('Renderer did not reach the Ludora workbench');
  ...
  process.stdout.write(`Ludora UI smoke captured ${output}\n`);
  app.quit();
}
  ...
  await Promise.all(
    projects
      ...
      .map((project) => projectStore.update(project.id, {
        status: 'stopped',
        activeTurnId: null,
        lastError: 'Ludora 上次退出时，该任务没有确认完成。请检查文件后再继续。',
      })),
  );
}
