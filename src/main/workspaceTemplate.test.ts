...
describe('createWorkspaceTemplate', () => {
  it('creates a project with a branded waiting preview and local Agent instructions', async () => {
    ...
    ).resolves.toContain('Ludora Game Builder');
    ...
    expect(agents).toContain('host-trusted generated image is required for every Ludora game');
    ...
    expect(skill.indexOf(NOOBI_HOST_RUNTIME_POLICY_START)).toBeLessThan(
      skill.indexOf('# Ludora Game Builder'),
    );
    ...
    expect(readme).toContain('Every Ludora run includes an animation needs assessment');
  ...
  it('atomically synchronizes authoritative FPS metadata and managed policy blocks without replacing user content', async () => {
    ...
    expect(skillAfter.indexOf(NOOBI_HOST_RUNTIME_POLICY_START)).toBeLessThan(
      skillAfter.indexOf('# Ludora Game Builder'),
    );
