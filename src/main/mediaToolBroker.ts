...
export const MEDIA_DYNAMIC_TOOLS: DynamicToolSpec[] = [
  {
    ...
    description: 'List validated game assets in this Ludora project. Returns workspace-relative paths only.',
    inputSchema: {
      type: 'object',
      properties: {
...
export class MediaToolBroker {
  ...
  async #handle(request: JsonRpcServerRequest): Promise<void> {
    ...
    try {
      ...
      if (!project) throw new ToolInputError('This tool call is not attached to an active Ludora project');
      ...
      switch (params.tool) {
        ...
        case 'noobi_model3d_generate':
          ...
          break;
        default:
          throw new ToolInputError('Unknown Ludora media tool');
      }
  ...
  return {
    fallback: {
      ...
      ...(result.fallback === 'codex-imagegen'
        ? { instruction: 'Invoke the Codex $imagegen skill now, then register the generated image with Ludora.' }
