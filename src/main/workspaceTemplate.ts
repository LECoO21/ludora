...
function projectAgents(project: WorkspaceProject): string {
  const content = `# Ludora Game Project
...
- Never edit \`.noobi/project.json\`; it is owned by the Ludora host.
...
description: Build, verify, and iterate a small playable browser game in a Ludora project.
---
...
The starter has no runtime dependency and can be served directly by Ludora. For development tooling:
...
## Ludora host runtime and media policy (managed, v${NOOBI_HOST_RUNTIME_POLICY_VERSION})
...
- Agents must not edit \`.noobi/project.json\` or this managed block; Ludora refreshes both before each Harness run.
...
function stripManagedRuntimePolicies(content: string): string {
  ...
  while (true) {
    ...
    if (end < 0 || (nestedStart >= 0 && nestedStart < end)) {
      throw new Error('Workspace contains a malformed Ludora host runtime policy block');
    }
