  ...
  return (
    <section>
      ...
      <div className="extension-list" aria-live="polite">
        {skills.map((item) => {
          const requiredByLudora = item.name.trim().toLowerCase() === 'imagegen';
          return (
            <article className={`extension-row${requiredByLudora ? ' is-required' : ''}`} key={item.id}>
              ...
              <div className="extension-copy">
                <div>
                  ...
                  {requiredByLudora ? <span className="source-badge ludora-required-badge">LUDORA REQUIRED</span> : null}
                </div>
                <p>{requiredByLudora ? 'Ludora 的默认图片生成与 API 失败回退依赖此 Skill；游戏制作流程中始终保持启用。' : item.description || '此 Skill 未提供说明。'}</p>
                {item.path ? <code title={item.path}>{item.path}</code> : null}
              </div>
              <label className="switch-control compact-switch" title={requiredByLudora ? 'Ludora 游戏制作必需，无法在这里停用' : undefined}>
                <span className="sr-only">{requiredByLudora ? `${item.name} 是 Ludora 必需 Skill，始终启用` : `${item.enabled ? '停用' : '启用'} ${item.name}`}</span>
                <input
                  type="checkbox"
                  checked={requiredByLudora || item.enabled}
                  disabled={requiredByLudora || !controller.supported || pending === item.id}
