...
Ludora 是一个面向独立开发者和创意团队的游戏制作 Agent。用户用自然语言描述游戏，Codex 在受控项目目录内完成策划、脚手架、素材接入、动画需求判断、代码实现、构建验证和持续迭代；产品负责把 Codex App Server 的线程、回合、工具、审批与文件变化转化成可理解的制作工作台。

新工程只继承旧 Ludora 的产品能力和前端信息架构，不继承旧项目的迁移状态、私有运行时封装或历史数据门禁。
...
- 初始化标准游戏工作区：`AGENTS.md`、Ludora Skill、项目元数据和可运行网页游戏入口。
...
- Codex Skills：读取原生 Skills 目录并即时启停，不删除 Skill 文件；固定素材门禁依赖的宿主 ImageGen 标记为 `LUDORA REQUIRED`，不可停用。
