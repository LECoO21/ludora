...
App Server 是 Codex 为富客户端提供的正式深度集成面，覆盖认证、会话历史、审批和流式 Agent 事件。Ludora 是交互式桌面产品，因此使用 App Server，而不是面向 CI/自动作业的 SDK。
...
1. 定位随 npm 包安装的 Codex 原生二进制；开发环境可使用 `LUDORA_CODEX_BIN` 覆盖。
