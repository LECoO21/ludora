...
try {
  await createWorkspaceTemplate(workspace, {
    id: 'image-smoke',
    name: 'Image Smoke',
    idea: 'Verify Codex ImageGen and Ludora workspace ingestion end to end.',
  ...
  if (asset.kind !== 'image' || asset.source !== 'generated') {
    throw new Error('Ludora did not ingest the generated image as a project asset.');
  }

  process.stdout.write(
    `Ludora ImageGen smoke passed: ${model.displayName}; ${asset.relativePath}; ${asset.size} bytes\n`,
  );
} finally {
