...
try {
  await createWorkspaceTemplate(workspace, {
    id: 'media-smoke',
    name: 'Media Smoke',
    idea: 'Verify Ludora dynamic media tools through the real Codex App Server.',
  ...
  threadId = await runtime.startThread({
    ...
    developerInstructions:
      'This is an automated Ludora media tool test. Use the provided dynamic tool exactly as requested. Do not run shell commands or edit files directly.',
  });
  ...
  process.stdout.write(
    `Ludora media smoke passed: ${model.displayName}; ${audio.relativePath}; ${audio.size} bytes\n`,
  );
} finally {
