...
export async function prepareSmokeHome(): Promise<SmokeHome> {
  const explicit = process.env.LUDORA_SMOKE_CODEX_HOME?.trim()
    || process.env.NOOBI_SMOKE_CODEX_HOME?.trim();
  if (explicit) {
    if (!isAbsolute(explicit)) throw new Error('LUDORA_SMOKE_CODEX_HOME must be absolute');
  ...
  try {
    ...
    throw new Error(
      `Unable to prepare isolated Codex smoke authentication from ${sourceHome}. Sign in with Codex first or set LUDORA_SMOKE_CODEX_HOME.`,
      { cause: error },
    );
  }
  return {
